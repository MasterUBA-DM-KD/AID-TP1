...
reacttable_defaults <- colDef(
  header = function(value) {
    gsub(".", " ", value, fixed = TRUE)
  },
  cell = function(value) {
    format(value, digits = 3, nsmall = 0)
  },
  align = "center",
  minWidth = 100,
  maxWidth = 10000,
...
```{r}
reactable(head(df), defaultColDef = reacttable_defaults)
```
...
```{r}
reactable(head(df_numeric), defaultColDef = reacttable_defaults)
```
...
reactable(numeric_summary,
  pagination = FALSE,
  defaultColDef = reacttable_defaults
)
```
...
for (col in numeric_cols) {
  var <- sym(col)
  p <- df_numeric %>%
    ggplot(aes(x = deposit, y = df_numeric[[col]])) +
    geom_boxplot(
      outlier.colour = "red",
      outlier.shape = 16,
      outlier.size = 2,
...
```{r}
p <-
  ggplot(data = table_freq, aes(x = marital, y = Freq, fill = deposit)) +
...
análisis de la base de datos, describiendo la base de datos, indicando la presencia de valores atípicos y las conclusiones a las que se abordó luego del análisis.
